...
_.extend(Metrics.prototype, Backbone.Events, {
  ...
  flush(isPageUnloading) {
    ...
    // For now, throw out data with containing known bad L1 timing errors.
    // In the future we may throw out L2 errors also.
    if (Object.values(filteredData.navigationTiming).some((x) => x === -1111)) {
      return;
    }
